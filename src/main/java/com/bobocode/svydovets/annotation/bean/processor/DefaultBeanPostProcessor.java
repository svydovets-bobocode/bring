...
public class DefaultBeanPostProcessor implements BeanPostProcessor {
    ...
    for (var entrySet : beanRegistry.getRootContext().entrySet()) {
      ...
      for (var field : beanType.getDeclaredFields()) {
        if (field.isAnnotationPresent(AutoSvydovets.class)) {
          var dependency = getBean(field.getType(), entrySet.getKey());
  ...
  private static void initField(Object beanObject, Field field, Object dependency) {
    try {
  ...
  private Object getBean(Class<?> type, String beanName) {
    return beanRegistry.getAllSingletons(type).get(beanName);
  }
}
